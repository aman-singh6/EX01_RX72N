/***********************************************************************************************************************
* 免責事項
* このソフトウェアはルネサス エレクトロニクス株式会社によって提供され、ルネサス製品での使用のみを目的としています。
* 他の用途には使用できません。このソフトウェアはルネサス エレクトロニクス株式会社の所有物であり、
* 著作権法を含むすべての適用法によって保護されています。
* このソフトウェアは「現状のまま」提供されており、ルネサスはこのソフトウェアに関して、明示的、黙示的、
* または法定のいかなる保証も行いません。商品性、特定目的への適合性、および非侵害についての保証を含むが、
* これに限定されないすべての保証を明示的に否認します。法律で禁止されていない最大限の範囲で、
* ルネサス エレクトロニクス株式会社およびその関連会社は、このソフトウェアに関連するいかなる理由による
* 直接的、間接的、特別、偶発的、または結果的な損害についても責任を負いません。
* ルネサスは、通知なしにこのソフトウェアに変更を加える権利を留保し、このソフトウェアの提供を中止することがあります。
* このソフトウェアを使用することにより、以下のリンクにアクセスすることで見つかる追加の条件に同意したものとみなされます。
...
************************************************************************************************************************
* 履歴 : 日付       バージョン 説明
*         : 08.04.2015 2.01    64M, 71M用。
*                              - NEWDATA、SENTDATA、MSGLOST、TRMABT、RECREQ、およびTRMREQビットに0を書き込む際、
*                              論理演算命令（AND）を使用せず、指定されたビットにのみ0を書き込み、
*                              他のビットには1を書き込んでから転送（MOV）命令を使用すること。
*                              RX64M/71M UM 43.2.8を参照。
*                              - R_CAN_TxCheck()およびR_CAN_TxStopMsg()を修正。注記
*                              "Bits SENTDATA and TRMREQ cannot be set to 0 simultaneously."を参照。RX64M/71M UM 43.2.8。
*         : 30.10.2015 2.02    - FIT用のコードパッケージの更新。
*                              - R_CAN_Create()：割り込みコールバックの引数を追加。
*                              - R_CAN_RxRead()を変更：IDEビットは混合モードでのみ有効。
*         : 09.06.2016 2.10    - 65Nを追加。
*                              - 混合IDモードのみで要求されたフレームタイプに応じてIDEビットを設定。
*                              - R_CAN_RxRead()を修正：混合モード：elseブロックが欠落。標準フレームIDがコピーされない。
*                              - R_CAN_Control()
*                                   o EXITSLEEP_CANMODEおよびENTERSLEEP_CANMODEケースで0ではなくch_nrを使用。
*                                   o OPERATE_CANMODEケース：Halt Statusビットが0であることも確認。
*         : 30.01.2017 2.11    - CAN_port_map_tにconstを追加し、インスタンス化にconstを追加する必要がなくなりました。
*                              - 65N 2MBでテスト実施。
*                              - 一部のGSCEコーディングガイドラインを実装。複数行変更。（プラグインを使用。）
*         : 14.08.2017 2.12    - CAN_ERS_ISR()関数を修正し、すべてのチャンネルの割り込みソースをチェック。
*         : 26.10.2018 2.13    - CAN2有効時にコンパイルエラーを引き起こすコメントフィールド"Function name:  CAN2_TXM2_ISR"を修正。
*         : 08.01.2019 2.15    - can_rx_callbackおよびcan_err_callbackのデフォルトコールバック割り当てを修正。
*         : 05.04.2019 3.00    - GCCおよびIARコンパイラのサポートを追加。
*         : 16.09.2019 3.11    - config_can_interrupts()関数、CAN0_TXM0_ISR()、CAN0_RXM0_ISR()、CAN1_TXM1_ISR()、CAN1_RXM1_ISR()、
*                                CAN2_TXM2_ISR()およびCAN2_RXM2_ISR()関数の上にCAN_USE_CAN0、CAN_USE_CAN1、CAN_USE_CAN2マクロを追加し、
*                                CAN0、CAN1またはCAN2を使用しない場合に割り込みベクタ番号が未定義になるのを防止。
*         : 30.12.2019 3.20    - API関数のコメントをDoxygenスタイルに変更。
*                              - アトミック制御のサポートを追加。
*                              - 第1世代デバイスのサポートを削除。
*                              - GSCEコーディングスタンダードRev.6.00に準拠するよう修正。
*         : 13.06.2020 4.00    - CAN FIFOのサポートを追加。
*                              - ピン設定に関連するソースコードを削除。
*                              - R_CAN_Control()のEXITSLEEP_CANMODEケースを修正。
*                              - R_CAN_RxSetMask()関数の処理を修正。
*         : 04.01.2021 4.10    - R_CAN_Create()でtxf_cb_funcがNULLの場合にcan_tx_callbackをcan_txf_callbackに変更。
*         : 01.04.2021 5.00    - 異なるチャンネルに異なるビットレートを設定するサポートを追加。
*         : 07.04.2021 5.10    - R_CAN_RxSetFIFOおよびR_CAN_RxSetFIFOXidのコメントを修正。
*                              - Doxygenコメントを更新。
*         : 08.09.2023 5.50    - GSCE Code Checker 6.50に従って更新。
*                              - WAIT_LOOPコメントを追加。
*         : 21.12.2023 5.60    - FIFOメールボックスモードでデータフレームとリモートフレームの両方を受信するサポートを追加。
*                              - FIFOメールボックスモードで拡張IDのリモートフレームを受信できない問題を修正。
***********************************************************************************************************************/
/******************************************************************************
...
Macro definitions
******************************************************************************/
/* これらのマクロは、いくつかのCANのwhileループがタイムアウトするかどうかを判断するためのものです。
タイムアウトした場合、canx_sw_err変数はゼロ以外の値になります。これは、CANペリフェラルとドライバのエラーがCPUを無期限にブロックするのを防ぐためです。 */
...
/* 拡張IDモードビットフラグとして機能する長整数のMSBのマスクを定義します。 */
/* 拡張IDは下位29ビットを占有するため、これを使用して上位3ビットをマスクします。 */
#define XID_MASK                (0xE0000000)

/* 標準IDを構成する11ビットのマスクを定義します。 */
#define SID_MASK                (0x000007FF)

/* MAX_CHANNELS が CAN チャンネルの数と一致していることを確認してください */
#define MAX_CHANNELS (sizeof(CAN_CHANNELS)/sizeof(can_st_ptr))

/******************************************************************************
/******************************************************************************
グローバル変数およびインポートされた関数（externs）
******************************************************************************/
/******************************************************************************
定数定義
******************************************************************************/
...
/* CAN割り込み用のユーザ関数ベクトル。これらはR_CAN_Create()によって設定されます。 */
#ifdef CAN2     /* デバイス上に3つのCANチャネルが存在します。 */
...
#else
#ifdef CAN1     /* デバイス上に2つのCANチャネルが存在します。 */
...
********************************************************************************************************************//**
* @brief CANペリフェラルを初期化します - ユーザー通信コールバック関数を設定し、CAN割り込みを構成し、異なるチャネルに異なるビットレートを設定し、
* メールボックスのデフォルトを設定し、CAN動作モードに入ります。\n
* この関数はCAN割り込みレベルとユーザーコールバックを設定します。この関数はR_CAN_SetBitrate()を呼び出し、デフォルトのマスクを設定します：フレームをマスクしません。
* @param[in] ch_nr - 使用するCANチャネル (MCU依存で0-2)。
* @param[in] mb_mode - 通常のメールボックス (0)、FIFO メールボックス (1)。
* @param[in] p_cfg - チャネル ch_nr のビットレートを構成する BRP、TSEG1、TSEG2、および SJW を含むデータ構造体へのアドレスです。
* @param[in] tx_cb_func - メールボックスの送信が完了したときにCANドライバによって呼び出されるアプリケーション内の関数の名前です。
* ポーリングモードを使用している場合、または何らかの理由で割り込みモードのコールバックを望まない場合は、NULLを指定します。
* @param[in] txf_cb_func - メールボックスが送信FIFOでの送信を完了するたび、または送信FIFOが送信完了により空になるたびにCANドライバによって呼び出されるアプリケーション内の関数の名前です。
* 割り込みモードのコールバックを望まない場合は、NULLを指定します。
* @param[in] rx_cb_func - メールボックスの受信が完了したときにCANドライバによって呼び出されるアプリケーション内の関数の名前です。
* ポーリングモードを使用している場合、または何らかの理由で割り込みモードのコールバックを望まない場合は、NULLを指定します。
* @param[in] rxf_cb_func - CANドライバによって、受信FIFO内のメールボックスが受信を完了したときや、受信FIFOが受信完了によってバッファ警告状態になったときに呼び出されるアプリケーション内の関数の名前です。
* 割り込みモードのコールバックを望まない場合は、NULLを指定します。
* @param[in] err_cb_func - CANドライバによってCANエラーが発生したときにアプリケーション内で呼び出される関数の名前です。
* ポーリングモードを使用している場合、または何らかの理由で割り込みモードのコールバックを望まない場合は、NULLを指定します。
* @retval R_CAN_OK                アクションが正常に完了しました。
* @retval R_CAN_SW_BAD_MBX        不正なメールボックス番号。
* @retval R_CAN_BAD_CH_NR         チャネル番号が存在しません。
* @retval R_CAN_BAD_MODE          モード番号が存在しません。
* @retval R_CAN_SW_RST_ERR        CANペリフェラルがリセットモードに入らなかった
* @retval R_CAN_MODULE_STOP_ERR   CANペリフェラル全体が停止状態（低電力）です。
* おそらくPRCRレジスタが使用されておらず、モジュール停止レジスタがロック解除されていない可能性があります。
* 他のR_CAN_Control()の戻り値も参照してください。
* @details この関数は、CANスリープモードからペリフェラルを起動し、CANリセットモードに設定します。次のデフォルト設定でメールボックスを構成します:\n
* メールボックスモードを設定: 通常のメールボックスモードまたはFIFOメールボックスモード。\n
* 新しいフレームが到着したときに未読のメールボックスデータを上書きします。\n
* デバイスをID優先（通常のCAN動作、オプションのメールボックス番号優先ではない）に設定します。\n
* すべてのメールボックスのマスクを無効に設定します。\n
* R_CAN_CreateはR_CAN_SetBitrate関数を呼び出し、r_can_rx_config.hでUSE_CAN_POLLがコメントアウトされている場合にCAN割り込みを構成します。\n
* 戻る前に、すべてのメールボックスをクリアし、ペリフェラルを動作モードに設定し、エラーをクリアします。
* @note ユーザーは、R_CAN_Create関数を呼び出す前に、p_cfg引数を通じてCANチャネルのビットレートを設定するためのボーレートプリスケーラ分周値とビットタイミング値を宣言する必要があります。
*/
                    ...
                    void (*err_cb_func)(void))
{
    ...
    /* ハードウェアやファームウェアの不具合により、CANペリフェラルブロックがプログラムをwhileループでブロック（ハング）させる可能性があります。
    これを防ぐために、whileループ内のソフトウェアタイマーがタイムアウトし、CPUが続行できるようにします。 */
    ...
    if (NULL == tx_cb_func)
    {
        /* ポーリングモードの場合でも、安全対策として割り込みをデフォルト関数に設定します。 */
        can_tx_callback[ch_nr] = universal_can_callback;
    }
    else
    {
        /* CAN送信割り込みが発生したときに実行するユーザー関数を設定します。 */
    ...
    {
        /* CAN受信割り込みが発生したときに実行するユーザー関数を設定します。 */
...
* 関数名: R_CAN_PortSet
********************************************************************************************************************//**
* @brief MCUおよびトランシーバーポートピンを構成します。この関数は、MCUおよびトランシーバーポートピンの構成を担当します。
* トランシーバーポートピン（Enableなど）は設計によって異なるため、この関数を変更する必要があります。
* この関数は、Listen OnlyなどのCANポートテストモードに入るためにも使用されます。
* @param[in] ch_nr - 使用するCANチャネル (MCU依存で0-2)。
* @param[in] action_type \n
* ポートアクション:\n
*  ENABLE                      CANポートピンとCANトランシーバを有効にします。\n
*  DISABLE                     CANポートピンとCANトランシーバを無効にします。\n
*  CANPORT_TEST_LISTEN_ONLY    リッスンオンリーモードに設定します。ACKやエラーフレームは送信されません。\n
*  CANPORT_TEST_0_EXT_LOOPBACK 外部バスとループバックを使用します。初期デバッグに便利です。別のテストセクションを参照してください。\n
*  CANPORT_TEST_1_INT_LOOPBACK 内部のメールボックス通信のみを使用します。初期デバッグに便利です。別のテストセクションを参照してください。\n
*  CANPORT_RETURN_TO_NORMAL    通常のポート使用に戻します。\n
* @retval R_CAN_OK                アクションが正常に完了しました。
* @retval R_CAN_SW_BAD_MBX        不正なメールボックス番号。
* @retval R_CAN_BAD_CH_NR         チャネル番号が存在しません。
* @retval R_CAN_BAD_ACTION_TYPE   アクションタイプが存在しません。
* @retval R_CAN_SW_HALT_ERR       CANペリフェラルがハルトモードに入らなかった。
* @retval R_CAN_SW_RST_ERR        CANペリフェラルがリセットモードに入らなかった。\n
* 他のR_CAN_Control()の戻り値も参照してください。
* @details 内部ループバックモードを使用しない限り（初期テストおよびデバッグ用）、この関数はボードのデフォルトポート設定関数（例： 'hwsetup'）が使用された後に呼び出されることを確認してください。\n
* 他の（デフォルトの）ボード設定コードによって設定されたMCU CANポートピンの出力高/低がバスに悪影響を与える可能性があることに注意してください。
* ノードのハードリセットが他のノードをエラーモードにする原因となることがあります。
* その理由は、CANがポートを再構成する前に、すべてのポートがデフォルトの出力高/低に設定されていたためです。
* そのようなコードは削除する必要があります。さもなければ、短期間の間、ポートが出力低/高になり、CANバスの電圧レベルを乱す可能性があります。\n
* トランシーバーに応じて、トランシーバーポートピンを変更/追加する必要がある場合があります。\n
*/
